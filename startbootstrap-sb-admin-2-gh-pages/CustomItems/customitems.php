...
<img src="images/fake.png" alt="Forest" width="500" height="500">
		<textarea id="header">Items/Supplier Report</textarea>
...
Phone: 661-555-5555
Summary:
A report on the expenditures per supplier per item.
<?php
include 'config2.php';
$sql =<<<EOF
...
while($row = pg_fetch_row($ret)) {
    $id = $row[0];
    $item = "select * from mostboughtitem($id)";
    $itemquery = pg_query($db,$item);
    $itemrow = pg_fetch_row($itemquery);
    $total = "select * from monthlytotal($id)";
...
while($row = pg_fetch_row($ret)) {
	...
	$totalm = "select * from totalpurchasenumber($id,$itemid)";
	...
	$numbers = preg_replace('/[^0-9]/', '', $totalrow[0]);
	$grandtotal = $grandtotal + $msrp;
    }
}

$grandtotal = money_format('%i', ($grandtotal/100));
$_SESSION['grandtotal'] = $grandtotal;


$sdate = $_SESSION['csdate2'];
$edate = $_SESSION['cedate2'];
$intsd = (int)$sdate;
$inted = (int)$edate;
...
while($row = pg_fetch_row($ret)) {
	...
	if($row[1] == $a) {
	    ...
	    $thetotal = $thetotal + $suptotal;
	    $suptotal = money_format('%i', ($suptotal/100));
	}
    }
}
$thetotal = money_format('%i', ($thetotal/100));
$_SESSION['thetotal'] = $thetotal;
?>
</textarea>
...
?>    
		...
		<tr>
		    <td class="meta-head">Grand Total Spent</td>
		    <td><div class="due">$<?php echo $_SESSION['thetotal'];?></div></td>
		</tr>
