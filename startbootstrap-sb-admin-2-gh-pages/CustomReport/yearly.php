...
<img src="images/fake.png" alt="Forest" width="500" height="500">
		<textarea id="header">Yearly Report</textarea>
...
Phone: 661-555-5555
Summary:
A report on the expenditures per yearly supplier.
<?php
include 'config2.php';
$sql =<<<EOF
...
$month;
$sql6 =<<<EOF
select * from supplier natural join
(select distinct supplierid from 
yearlycomplete) as test;
EOF;
$ret = pg_query($db,$sql6);
while($row = pg_fetch_row($ret)) {
    $id = $row[0];
    $item = "select mostboughtyearly($id)";
    $itemquery = pg_query($db,$item);
    $itemrow = pg_fetch_row($itemquery);
    $total = "select yearlytotal($id)";
...
?>
		...
		<textarea id="customer-title">Company Name: <?php echo $_SESSION['name'];?></textarea>
...
echo "<td class=\"meta-head\">Year of</td>";
...
?>    
		...
		<tr>
		    ...
		    <td><div class="due">$<?php echo $_SESSION['yamount'];?></div></td>
		</tr>
...
while($row = pg_fetch_row($ret)) {
    ...
    $total = "select * from yearlytotal($id)";
    $totalquery = pg_query($db, $total);
    $totalrow = pg_fetch_row($totalquery);
    $num = preg_replace('/[^0-9]/','',$row[6]);
    $totals = $row[2] * $num;
    ...
    $totalp = "select yearlytotalpurchasenumber($id)";
    $totalpquery = pg_query($db, $totalp);
    $psum = pg_fetch_row($totalpquery);
    echo"<tr class=\"item-row\">";
    echo"<td class=\"item-name\"><div class=\"delete-wpr\"><p>$row[1]</p></div></td>";
    echo "<td class=\"description\"><p>$row[2]</p></td>";
    echo"<td><p class=\"cost\">                           $psum[0]</p></td>";
    echo    "<td><p class=\"qty\">                        $itemrow[0]</p></td>";
    echo		      "<td><span class=\"price\">$totalrow[0]</span></td>";
    echo "\n";
    echo "</tr>";
    $numbers = preg_replace('/[^0-9]/', '', $totalrow[0]);
    $month = $month + $numbers;
