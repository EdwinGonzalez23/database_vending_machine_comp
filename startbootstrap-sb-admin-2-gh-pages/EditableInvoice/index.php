...
while($row = pg_fetch_row($ret)) {
	if($_SESSION['sid'] == $row[0]) {
		...
		echo   " <td><textarea>$invnum</textarea></td>";
...
foreach($_SESSION['SBCScart'] as $SBCSproduct)
{
    ...
    echo"<td class=\"item-name\"><div class=\"delete-wpr\"><textarea>$prod</textarea></div></td>";
    echo "<td class=\"description\"><textarea>Test Description Here</textarea></td>";
    echo"<td><textarea class=\"cost\">                           $price</textarea></td>";
    echo    "<td><textarea class=\"qty\">                        $quantity</textarea></td>";
    echo		      "<td><span class=\"price\">$$total</span></td>";
    echo "\n";
    echo "</tr>";
    $amount = $amount + $total;
    $_SESSION['amount'] = $amount;
