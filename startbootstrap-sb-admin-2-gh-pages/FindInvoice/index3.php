...
//find day placed
$oid = $_SESSION['test'];
$day = "select * from placesorder where orderid = $oid";
$dayq = pg_query($db, $day);
$dayrow = pg_fetch_row($dayq);
$_SESSION['date'] = $dayrow[3];
$grandamount;

$sql =<<<EOF
select * from ordercontains natural join itemtype;
EOF;
$grandtotal = "select * from ordercontains natural join itemtype where orderid =$oid";
$grandtotalq = pg_query($db, $grandtotal);
$ret = pg_query($db,$sql);
while($row = pg_fetch_row($grandtotalq)) {
  //  if($row[1] == $_SESSION['test']) {
    $num = preg_replace('/[^0-9]/','',$row[3]);
	$temp = $row[2] * $num;
	$grandamount = $grandamount + $temp;
//	$sum =  money_format('%i', ($totals/100));
//	$amount = $amount +  $sum;
    //	$_SESSION['amount'] = $amount;
//    }
}
	$grandamount =  money_format('%i', ($grandamount/100));
    	$_SESSION['grand'] = $grandamount;
?>
</textarea>
...
?>    
		...
		<tr>
		    ...
		    <td><p id="date"><?php echo $_SESSION['date'];?></p></td>
		</tr>
		<tr>
		    <td class="meta-head">Amount Due</td>
		    <td><div class="due">$<?php echo $_SESSION['grand'];?></div></td>
		</tr>
