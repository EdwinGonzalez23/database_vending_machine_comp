...
foreach($_SESSION['SBCScart'] as $SBCSitem) {
    ...
    $q3 = "INSERT INTO ordercontains(itemtypeid,orderid,numitemtype,itemtypeprice,expdate) values ($itemid,$latestOID[0],500,money'5',date'05-05-2015')";
...
while($row = pg_fetch_row($ret)) {
    ...
    echo		"<input class=\"form-control quantity\" name=\"quantity\" type=\"text\" onkeypress=\"return isNumberKey(event)\" maxlength=\"2\" value=\"1\"> $row[1]";
...
<!-- name, phone, street, citty, state, zip-->
  ...
  <!--  <input type="text" name="name" class="form-control" id="inputEmail1" placeholder="supplier name"</div>
					  ...
					  <div class="form-group">
						<div>
						<button type="submit" class="btn btn-success pull-right"><a href = "http://delphi.cs.csub.edu/~egonzale/startbootstrap-sb-admin-2-gh-pages/EditableInvoice/index.php" target = "_blank">Give Order</a></button>
